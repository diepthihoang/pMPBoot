...
void IQTree::initSettings(Params &params) {
    ...
    candidateTrees.init(this->aln, 5);
    
    // exit(0);
    intermediateTrees.init(this->aln, 200);
    ...
    if (0<nParTrees) {
        ...
        for (int treeNr = 1; treeNr <= nParTrees; ++treeNr) {
            ...
            if (params->start_tree == STT_PLL_PARSIMONY) {
                ...
                pllComputeRandomizedStepwiseAdditionParsimonyTree(pllInst, pllPartitions, 1);
                //resetBranches(pllInst);
...
//            int pos = addTreeToCandidateSet(curParsTree, -DBL_MAX, false, MPIHelper::getInstance().getProcessID());
            ...
            // cout << treeNr << " " << pos << " " << candidateTrees.size() << " " << candidateTrees.max_size() <<  endl;
...
void IQTree::doParsimonyHillClimb() {
    initCandidateTreeSet(100, 100);
    curScore = -computeParsimony();
    params->unsuccess_iteration = (aln->at(0).size() + 99) / 100 * 100;

    double startTime = getRealTime(), startCPU = getCPUTime();
    int iterations_passed = 0;

    int bestScoreAchieved = getBestScore(), lastUpdated = 0;
    for(;iterations_passed < lastUpdated + params->unsuccess_iteration;) {
        iterations_passed += 1;
        ...
        if (curScore > bestScoreAchieved) {
            lastUpdated = iterations_passed;
            bestScoreAchieved = curScore;

            cout << "=================================================================" << endl;
            cout << "|                       BEST SCORE UPDATED                      |" << endl; 
            cout << "=================================================================" << endl;
            cout << endl;
            cout << -curScore << endl;
            cout << endl;
    ...
    double endTime = getRealTime(), endCPU = getCPUTime();


    cout << endl;
    cout << candidateTrees.getBestScore() << " " << curScore << " " << candidateTrees.size() << endl;
    cout << endl;

    cout << "CPU time used: " << endCPU - startCPU << endl;
    cout << "Total real time used: " << endTime - startTime << " for " << iterations_passed << " iterations." << endl;
    if (iterations_passed > 0) {
        cout << "On average, each search iteration took: " << (endTime - startTime) / iterations_passed << endl;
    }
}
...
//    }
    ...
    cout << getCheckpoint()->getBool("finishedCandidateSet") << endl;
